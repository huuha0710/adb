!function(){function f(){var
a=document.createElement("div");a.id="Adblock";a.innerHTML='
<div class="inner">
  <div class="header"><h2>Ad Blocker Detected</h2></div>
  <div class="isi">
    <p>Please consider supporting us by disabling your ad blocker</p>
    <div class="tombol">
      <button class="1 active">Adblock</button
      ><button class="2">Adblock Plus</button>
    </div>
    <div class="fixblock">
      <div class="1 active">
        <ol>
          <li>
            Click on the AdBlock icon in your browser<br /><img
              src="https://1.bp.blogspot.com/-cZJbVZ0XL-s/YVgCJdl7BRI/AAAAAAAAM3Q/GNijdINg1H4WTn8oWOURSs9aWtE6VZdCgCLcBGAsYHQ/s0/t%25E1%25BA%25A3i%2Bxu%25E1%25BB%2591ng.png"
              alt="Adblock"
            />
          </li>
          <li>
            Choose, Don't run on pages on this domain<br /><img
              src="https://1.bp.blogspot.com/-dBDqDT_1J2w/YVgCf6VZPhI/AAAAAAAAM3Y/T7ZaIHBljLE6tfk3IRXuV76Pm_942yOmACLcBGAsYHQ/s0/11.PNG"
              alt="Adblock"
              width="300px"
            />
          </li>
          
          <li>
            The browser icon should have turned grey<br /><img
              src="data:image/png;base64,"
              alt="Adblock"
            />
          </li>
          <li>Refresh the page if it didn\'t refresh automatically. Thanks!</li>
        </ol>
      </div>
      <div class="2">
        <ol>
          <li>
            Click on the AdBlock Plus icon in your browser<br /><img
              src="https://1.bp.blogspot.com/-klwKfzM6PkU/YVgEuaXPkDI/AAAAAAAAM3g/iM543YmCbwscGkad_cEBGl6pMRMDOp3ewCLcBGAsYHQ/s0/222.png"
              alt="Adblock"
            />
          </li>
          <li>
            Click on "Enabled on this site" position<br /><img
              src="https://1.bp.blogspot.com/-Ex1pxY73D-w/YVgFtgbf6UI/AAAAAAAAM30/FO5tP39zo9QwHrRqMR-br8vJ-VDNZI2AACLcBGAsYHQ/s0/222222%2B%25281%2529.png"
              alt="Adblock"
              width="250px"
            />
          </li>
          
          <li>
            The browser icon should have turned grey<br /><img
              src="https://1.bp.blogspot.com/-CU2xyFYfpoQ/YVgF4QNd-XI/AAAAAAAAM38/028gzp73Lx0FqPJ7z0Bcg4rkFdNb6RCKgCLcBGAsYHQ/s0/t%25E1%25BA%25A3i%2Bxu%25E1%25BB%2591ng%2B%25281%2529.png"
              alt="Adblock"
            />
          </li>
          <li>Refresh the page if it didn\'t refresh automatically. Thanks!</li>
        </ol>
      </div>
    </div>
  </div>
</div>
'; document.body.append(a);document.body.style.overflow="hidden";var
b=a.querySelectorAll("button");a.querySelector(".close");var
d=a.querySelectorAll(".fixblock >
div");for(a=0;a<b.length;a++)b[a].addEventListener("click",function(a){a.preventDefault();a=this.getAttribute("class").split("
")[0];for(var
c=0;c<d.length;c++)d[c].classList.remove("active"),b[c].classList.remove("active");b[a-1].classList.add("active");d[a-1].classList.add("active")})}var
b=document.createElement("script");b.type="text/javascript";
b.async=!0;b.src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js";b.onerror=function(){f();window.adblock=!0};var
e=document.getElementsByTagName("script")[0];e.parentNode.insertBefore(b,e)}();
